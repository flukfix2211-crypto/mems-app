...
## นำเข้าข้อมูลเก่าจากชีทเดิม (Migration)

ไฟล์ `ImportLegacyData_Aug2026.gs` เป็นสคริปต์นำเข้าครั้งเดียว สำหรับย้ายประวัติยืม-คืน
จากชีทเก่า (1–26 ส.ค. 2569, 1,139 รายการ) เข้าสู่ Sheet **ยืม-คืน** ของระบบใหม่

วิธีใช้: เปิด Apps Script editor ของโปรเจกต์ MEMs → เพิ่มไฟล์ script ใหม่ → วางโค้ดจาก
`ImportLegacyData_Aug2026.gs` → รันฟังก์ชัน `importLegacyBorrowRecords` ครั้งเดียว
(ดูรายละเอียด/ข้อควรระวังในคอมเมนต์หัวไฟล์)

มี 8 รายการที่ข้อมูลต้นฉบับผิดปกติ (เลขเครื่องพิมพ์ผิด/สลับคอลัมน์) — สคริปต์แก้ไขเท่าที่ทำได้
และใส่หมายเหตุกำกับไว้ในคอลัมน์ "หมายเหตุ" ของแถวนั้น กรุณาตรวจสอบเพิ่มเติมหลังนำเข้า
